use crate::shared::counter_handler::*;
...
define_class! {
    ...
    pub struct CounterAppDelegate;

    impl CounterAppDelegate {
        ...
        unsafe fn create_application(&self, _object: Option<&AnyObject>) {
            ...
            assert!(app.isKindOfClass(CounterApplication::class()));
            assert!(
                app.delegate()
                    .unwrap()
                    .isKindOfClass(CounterAppDelegate::class())
            );
    ...
    unsafe impl NSObjectProtocol for CounterAppDelegate {}

    unsafe impl NSApplicationDelegate for CounterAppDelegate {
        ...
        unsafe fn application_should_handle_reopen(&self, _sender: &NSApplication, _has_visible_windows: Bool) -> Bool {
            if let Some(handler) = CounterHandler::instance() {
                let mut handler = handler.lock().expect("Failed to lock CounterHandler");
    ...
    unsafe impl NSUserInterfaceValidations for CounterAppDelegate {
...
impl CounterAppDelegate {
    fn new(mtm: MainThreadMarker) -> Retained<Self> {
        let this = CounterAppDelegate::alloc(mtm).set_ivars(());
        unsafe { msg_send![super(this), init] }
    }
}

/// Instance variables of `CounterApplication`.
#[derive(Default)]
pub struct CounterApplicationIvars {
...
define_class!(
    ...
    unsafe impl CrAppControlProtocol for CounterApplication {
    ...
    unsafe impl CrAppProtocol for CounterApplication {
        #[unsafe(method(isHandlingSendEvent))]
        unsafe fn _is_handling_send_event(&self) -> Bool {
            self.ivars().handling_send_event.get()
        }
    }
    unsafe impl CefAppProtocol for CounterApplication {}
);
...
pub fn setup_counter_application() {
    // Initialize the CounterApplication instance.
    // SAFETY: mtm ensures that here is the main thread.
    let _ = CounterApplication::shared_application();

    // If there was an invocation to NSApp prior to here,
    // then the NSApp will not be a CounterApplication.
    // The following assertion ensures that this doesn't happen.
    assert!(
        NSApp(MainThreadMarker::new().expect("Not running on the main thread"))
            .isKindOfClass(CounterApplication::class())
    );
}

pub fn setup_counter_app_delegate() -> Retained<CounterAppDelegate> {
    let mtm = MainThreadMarker::new().expect("Not running on the main thread");

    // Create the application delegate.
    let counter_delegate = CounterAppDelegate::new(mtm);
    let delegate_proto =
        ProtocolObject::<dyn NSApplicationDelegate>::from_retained(counter_delegate.clone());
    let app = NSApp(MainThreadMarker::new().expect("Not running on the main thread"));
    assert!(app.isKindOfClass(CounterApplication::class()));
    ...
    unsafe {
        counter_delegate.performSelectorOnMainThread_withObject_waitUntilDone(
