...
static HELLO_HANDLER_INSTANCE: OnceLock<Weak<Mutex<HelloHandler>>> = OnceLock::new();
...
impl HelloHandler {
    ...
    pub fn instance() -> Option<Arc<Mutex<Self>>> {
        HELLO_HANDLER_INSTANCE.get().and_then(|weak| weak.upgrade())
    }

    pub fn new(is_alloy_style: bool) -> Arc<Mutex<Self>> {
        Arc::new_cyclic(|weak| {
            if let Err(instance) = HELLO_HANDLER_INSTANCE.set(weak.clone()) {
    ...
    pub fn show_main_window(&mut self) {
        ...
        if currently_on(thread_id) == 0 {
            ...
            let this = self
                .weak_self
                .upgrade()
                .expect("Weak reference to HelloHandler is None");
...
wrap_client! {
    pub struct HelloHandlerClient {
        inner: Arc<Mutex<HelloHandler>>,
    }

    impl Client {
        fn display_handler(&self) -> Option<DisplayHandler> {
            Some(HelloHandlerDisplayHandler::new(self.inner.clone()))
        }

        fn life_span_handler(&self) -> Option<LifeSpanHandler> {
            Some(HelloHandlerLifeSpanHandler::new(self.inner.clone()))
        }

        fn load_handler(&self) -> Option<LoadHandler> {
            Some(HelloHandlerLoadHandler::new(self.inner.clone()))
        }
    }
}

wrap_display_handler! {
    struct HelloHandlerDisplayHandler {
        inner: Arc<Mutex<HelloHandler>>,
    }
...
wrap_life_span_handler! {
    struct HelloHandlerLifeSpanHandler {
        inner: Arc<Mutex<HelloHandler>>,
    }
...
wrap_load_handler! {
    struct HelloHandlerLoadHandler {
        inner: Arc<Mutex<HelloHandler>>,
    }
